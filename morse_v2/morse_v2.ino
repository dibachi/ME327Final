...
//  char *current_character = morse_sequence[i];
  ...
  if (state != 0) {
    ...
    // }
    if (state == 1) {
      pulldown_force_stiffness = 0; //100
    } else if (state == 2) {
      pulldown_force_stiffness = 0; //50 
    } else {
      pulldown_force_stiffness = 0; //25
    }
  ...
  if (xh > contact_position) {
    ...
    if (intervalContact_l > 100*(dot - 0.5*unit_time) && intervalContact_l < 100*dash) { //  + unit_time
      Serial.print("dot");
      Serial.print(",");
    } else {
      Serial.print("dash");
